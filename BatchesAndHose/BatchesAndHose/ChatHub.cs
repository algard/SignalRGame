...
public class ChatHub : Hub
{
    ...
    public void AddNewPlayer(string name, string avatar, string image)
    {
		...
		//get images for avatar
		List<string> avatarURLs = new List<string>();

		XmlDocument avatarDoc = new XmlDocument();
		urlDoc.Load("http://api.flickr.com/services/rest/?&method=flickr.photos.search&api_key=8531f340cce6465f6570d9933698ec52&text=" + avatar);

		foreach (XmlElement photo in avatarDoc.SelectNodes("/rsp/photos/photo"))
		{
			avatarURLs.Add("http://farm" + photo.GetAttribute("farm") + ".staticflickr.com/" + photo.GetAttribute("server") + "/" + photo.GetAttribute("id") + "_" + photo.GetAttribute("secret") + "_s.jpg");
		}

	    //avatar = avatarURLs[0];	// replace avatar search term with avatar URL
